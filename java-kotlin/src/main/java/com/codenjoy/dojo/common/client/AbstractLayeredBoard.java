package com.codenjoy.dojo.common.client;
...
import com.codenjoy.dojo.common.services.PointImpl;
import com.codenjoy.dojo.common.services.Point;
import com.codenjoy.dojo.common.services.printer.CharElements;
...
public abstract class AbstractLayeredBoard<E extends CharElements> implements ClientBoard {
    ...
    protected boolean isAt(int numLayer, int x, int y, E element) {
        if (PointImpl.pt(x, y).isOutOf(size)) {
            return false;
        }
    ...
    protected List<E> getNear(int numLayer, int x, int y) {
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
                if (PointImpl.pt(x + dx, y + dy).isOutOf(size)) {
                    continue;
                }
                if (dx == 0 && dy == 0) {
                    continue;
    ...
    public boolean isOutOfField(int x, int y) {
        return PointImpl.pt(x, y).isOutOf(size);
    }
